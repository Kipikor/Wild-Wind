...
namespace McpUnity.Unity
{
    ...
    public class McpUnityServer : IDisposable
    {
        ...
        private McpUnityLocalWebSocketServer _webSocketServer;
        ...
        public void InstallServer()
        {
            ...
            string buildEntryPath = Path.Combine(buildPath, "index.js");
            if (File.Exists(buildEntryPath))
            {
                McpLogger.LogInfo("MCP Node.js server build found; skipping automatic npm install.");
                return;
            }
        ...
        private StartServerResult StartServerInternal(bool logAddressInUseAsError)
        {
            ...
            McpUnityLocalWebSocketServer webSocketServer = null;
            try
            {
                int connectionGeneration = Interlocked.Increment(ref _connectionGeneration);
                var configuredHost = string.IsNullOrWhiteSpace(McpUnitySettings.Instance.Host)
                    ? "127.0.0.1"
                    : McpUnitySettings.Instance.Host.Trim();
                var host = McpUnitySettings.Instance.AllowRemoteConnections ? "0.0.0.0" : configuredHost;
                webSocketServer = new McpUnityLocalWebSocketServer(host, McpUnitySettings.Instance.Port, this, connectionGeneration);
        ...
        private void CleanupFailedStart(McpUnityLocalWebSocketServer webSocketServer)
        {
        ...
        private void CloseAllClients(ushort closeCode, string reason)
        {
            ...
            try
            {
                var sessionCount = Clients.Count;
                _webSocketServer.CloseAllClients(closeCode, reason);
                McpLogger.LogInfo($"Closed {sessionCount} client connection(s) with code {closeCode}: {reason}");
            }
            catch (Exception ex)
            {
        ...
        internal void HandleWebSocketMessage(string data, Action<string> sendResponse)
        {
            if (!ShouldTrackClient(_activeConnectionGeneration))
            {
                sendResponse(CreateResponse(null, CreateErrorResponse("Server is restarting", "server_restarting")).ToString(Formatting.None));
                return;
            }

            _mainThreadActions.Enqueue(() => HandleMessageAsync(data, sendResponse));
        }

        private void PumpMainThreadActions()
        {
            while (_mainThreadActions.TryDequeue(out var action))
            {
                ...
                {
                    McpLogger.LogError($"Error in MCP main thread action: {ex.Message}");
                }
            }
        }

        private async void HandleMessageAsync(string data, Action<string> sendResponse)
        {
            try
            {
                ...
                catch (JsonReaderException jre)
                {
                    ...
                    sendResponse(CreateResponse(null, CreateErrorResponse($"Invalid JSON format: {jre.Message}", "invalid_json")).ToString(Formatting.None));
                    return;
                }
                ...
                else if (TryGetTool(method, out var tool))
                {
                    EditorCoroutineUtility.StartCoroutineOwnerless(ExecuteTool(tool, parameters, tcs));
                }
                else if (TryGetResource(method, out var resource))
                {
                ...
                var jsonRpcResponse = CreateResponse(requestId, responseJson);
                var responseStr = jsonRpcResponse.ToString(Formatting.None);
            ...
            catch (Exception ex)
            {
                ...
                sendResponse(CreateResponse(null, CreateErrorResponse($"Internal server error: {ex.Message}", "internal_error")).ToString(Formatting.None));
            }
        }
        ...
        private static JObject CreateErrorResponse(string message, string errorType)
        {
            return new JObject
            {
                ["error"] = new JObject
